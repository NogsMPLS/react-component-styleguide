 ...
 */
function RES (input, opts) {
  opts = opts || {}

  this.log = require('./logger')('res-lib', {debug: opts.verbose || false})
  ...
  this.stageDir = path.resolve(__dirname, '../res-tmp')
}

RES.prototype.cleanStageDir = function () {
  fs.ensureDirSync(this.stageDir)
  fs.removeSync(this.stageDir)
  fs.ensureDirSync(this.stageDir)
}

RES.prototype.cleanOutDir = function () {
 ...
 * @returns {Object}
 */
RES.prototype.readConfig = function (file) {
 ...
 */
RES.prototype.getReactPropDocFiles = function () {
 ...
 * @returns {Promise}
 */
RES.prototype.copy = function (src, dest) {
 ...
 * @returns {Promise}
 */
RES.prototype.copyOptsFiles = function () {
  var copy = this.copy
 ...
 * @returns {string[]}
 */
RES.prototype.extractFiles = function (ext) {
 ...
 * @returns {Promise}
 */
RES.prototype.createHtmlFile = function () {
 ...
 * @returns {Promise}
 */
RES.prototype.createContentsFile = function () {
  var self = this

  var contentsInter = path.join(this.stageDir, '/component-requires.js')

  var data = [];
  this.input.forEach(function (file) {
    self.log.debug({file: file}, 'styleguide input file')
    var fileObj = {
      path: slash(file),
      module: require(slash(file))
    }
    data.push(JSON.stringify(fileObj))
  })

  var dataExport = 'module.exports = [' + data + ']'
  fs.outputFileSync(contentsInter, dataExport)
...
RES.prototype.getWebpackConfig = function () {
  ...
  var loaders = [
    ...
    deepmerge({
      // for when res is included in another project
      test: /\.jsx?$/,
      include: [appDir]
    }, babelLoaderCfg),
    {
 ...
 * @returns {Promise}
 */
RES.prototype.generate = function (cb) {
  var self = this
